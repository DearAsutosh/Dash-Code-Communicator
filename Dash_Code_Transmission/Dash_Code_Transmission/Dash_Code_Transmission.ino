...
void onRelease(Btn &me, unsigned long now) {
  ...
  // Check if both released nearly simultaneously → SPACE
  if (A.stableState == HIGH && B.stableState == HIGH) {
    long diff = (long)A.lastDebounceTime - (long)B.lastDebounceTime;
    if (abs(diff) <= 100) {   // both nearly together
